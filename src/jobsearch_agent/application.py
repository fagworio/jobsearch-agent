...
TRANSITIONS: dict[ApplicationState, set[ApplicationState]] = {
    ...
    ApplicationState.NEEDS_CAPTCHA: {ApplicationState.POLICY_BLOCKED},
...
class ApplicationService:
    ...
    def transition(self, application_id: str, target: ApplicationState, event: str, payload: dict[str, Any] | None = None) -> Application:
        ...
        if target not in TRANSITIONS[application.state]:
            raise ApplicationDomainError(f"invalid application transition: {application.state.value} -> {target.value}")
...
def evaluate_safety_gate(context: ApplicationContext, required_fields: list[Any] | None = None) -> ApplicationReadiness:
    ...
    if fit_blockers or not resume_ok:
        decision = ApplicationState.REJECTED
    checks.append({"gate": "policy", "result": "review" if decision == ApplicationState.READY_FOR_REVIEW else "pass", "evidence": context.policy.autonomy})
    return ApplicationReadiness(decision, decision in {ApplicationState.READY_FOR_REVIEW, ApplicationState.READY_TO_APPLY}, checks, blockers)
