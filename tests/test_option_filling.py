...
class _Option:
    def __init__(self, label: str, clicks: list[str]):
        self.label = label
        self._clicks = clicks
...
class _ListBox:
    def __init__(self, box_id: str, labels: list[str], clicks: list[str], *, visible: bool = True):
        self.box_id = box_id
        self.clicks = clicks
    ...
    def set_labels(self, labels: list[str]) -> None:
        self.options = [_Option(label, self.clicks) for label in labels]
...
class _Page:
    def __init__(self, boxes: list[_ListBox]):
        self.boxes = boxes

    def get_by_role(self, _role: str) -> _Collection:
        return _Collection(self.boxes)
...
class _ComboLocator:
    ...
    def __init__(self, listbox: _ListBox, *, field_id: str = "q-notice", filters: dict[str, list[str]] | None = None):
    ...
    def fill(self, value: str) -> None:
        ...
        self.listbox.set_labels(self.filters.get(value, []))
...
def test_an_answer_already_identical_to_the_option_is_clicked_without_typing():
    clicks: list[str] = []
    listbox = _ListBox("react-select-q-listbox", ["Social media", "Job board", "Other"], clicks)
...
def test_a_city_typeahead_is_typed_only_after_the_open_list_comes_back_empty():
    ...
    assert locator.queries == ["Betim, Minas Gerais, Brazil", "Betim"]
